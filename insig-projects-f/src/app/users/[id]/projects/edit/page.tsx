...
import { ProjectUpdateRq, projectUpdateSchema } from "@/lib/projects/project.schema"
...
export default function EditProjectPage() {
  ...
  const [project, setProject] = useState<ProjectItemRs | null>(null)
  const [isLoadingProject, setIsLoadingProject] = useState(true)

  // Estados para validación en tiempo real
  const [formData, setFormData] = useState<ProjectUpdateRq>({
    title: "",
    status: "SUCCESS",
    startDate: "",
    endDate: "",
  ...
  type FormFields = keyof ProjectUpdateRq
  ...
  const loadProject = useCallback(async () => {
    if (!projectId) {
      toast.error("ID de proyecto no encontrado")
    ...
    try {
      ...
      setFormData({
        ...
        timeSpentHours: projectData.timeSpentHours,
  ...
  const updateProjectAction = async (
    ...
    if (!projectId) {
      setServerMessage("ID de proyecto no encontrado")
      return
    }
    ...
    try {
      ...
      const apiResponse = await updateProject(projectId, data)
      ...
      toast.success("Proyecto actualizado", {
        description: apiResponse.message || "El proyecto ha sido actualizado exitosamente.",
      })
  ...
  const [, submitAction, isPending] = useActionState(updateProjectAction, null)

  // Validación en tiempo real
  const validateField = useCallback((field: string, value: unknown) => {
    const tempData = { ...formData, [field]: value }
    const result = projectUpdateSchema.safeParse(tempData)
  ...
  // Loading state mientras carga el proyecto
  if (isLoadingProject) {
    return (
        ...
        <Card className="bg-white dark:bg-gray-800 border-blue-200 dark:border-gray-700 shadow-lg">
          <CardHeader>
            <CardTitle className="text-2xl text-blue-900 dark:text-blue-100">
              Actualizar Proyecto: {project.title}
            </CardTitle>
          </CardHeader>
          <CardContent>
            {/* Mensaje general */} 
